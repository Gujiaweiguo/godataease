...
func (h *SubjectHandler) Query(c *gin.Context) {
	...
	if err != nil {
		response.Error(c, "500000", "Failed to query subjects: "+err.Error())
		return
	}
...
func (h *SubjectHandler) Update(c *gin.Context) {
	...
	if req.ID == "" {
		...
		if err == nil && existing != nil {
			response.Error(c, "500000", "名称已经存在")
			return
		}
		...
		if err := h.repo.Create(subject); err != nil {
			response.Error(c, "500000", "Failed to create subject: "+err.Error())
			return
		}
	} else {
		// Update
		...
		if err != nil {
			response.Error(c, "500000", "Subject not found")
			return
		}
		...
		if err := h.repo.Update(subject); err != nil {
			response.Error(c, "500000", "Failed to update subject: "+err.Error())
			return
		}
	}
...
func (h *SubjectHandler) Delete(c *gin.Context) {
	...
	if err := h.repo.Delete(id); err != nil {
		response.Error(c, "500000", "Failed to delete subject: "+err.Error())
		return
	}
	response.Success(c, nil)
}
