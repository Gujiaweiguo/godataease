...
说明：当前开发编排默认启动 `godataease-app + godataease-redis`，MySQL 需提前可用，并且在 `my-net` 网络里可通过主机名 `mysql8` 访问。
若你的 MySQL 主机名不是 `mysql8`，请调整 `infra/compose/docker-compose.yml` 中 `godataease-app.environment.DATABASE_HOST`。
